...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByEmail(email: string, password: string): Promise<User | undefined> {
    ...
    async createCustomRequest(images: string, comment: string, product_id: string): Promise < User | undefined > {
  try {
    const query = {
      text: `INSERT INTO bouquetbar.custom (images, comment, product_id) VALUES ($1, $2, $3) RETURNING *;`,
      values: [images, comment, product_id]
    };
    console.log('Executing query:', query);
...
const query = {
  text: `
        INSERT INTO bouquetbar.products (
          ...
          "inStock", featured, isCustom, image, imagefirst, imagesecond, 
...
return {
  ...
  // Save landing contact (name, email, phone)
  async addLandingContact(contact: { name: string; email: string; phone: string }): Promise < any > {
  try {
...
const insertQuery = `
        INSERT INTO bouquetbar.landing_contacts (name, email, phone)
        VALUES ($1, $2, $3)
        RETURNING *;
      `;
const result = await db.query(insertQuery, [contact.name || null, contact.email, contact.phone || null]);
return { success: true, contact: result.rows[0] };
  ...
  try {
    ...
    const query = `SELECT id, name, email, phone, created_at FROM bouquetbar.landing_contacts ORDER BY created_at DESC;`;
    const result = await db.query(query);
    return result.rows;
  } catch(error) {
