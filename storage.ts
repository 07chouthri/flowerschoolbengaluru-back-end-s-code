...
export interface IStorage {
  ...
  addLandingContact(contact: { name: string; email: string; phone: string }): Promise<any>;
...
export class MemStorage implements IStorage {
  ...
  private landingContacts: Array<{ id: string; name: string; email: string; phone: string; createdAt: Date }> = [];
  ...
  async addLandingContact(contact: { name: string; email: string; phone: string }): Promise<any> {
    const id = randomUUID();
    const now = new Date();
    const record = { id, name: contact.name, email: contact.email, phone: contact.phone, createdAt: now };
