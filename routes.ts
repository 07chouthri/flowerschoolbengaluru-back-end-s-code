...
import { getAllCustomRequests } from "./database-storage";
...
app.post('/api/admin/custom-requests', async (req, res) => {
  try {
    const { images, comment, product_id } = req.body;
    console.log('Received custom request:', { images, comment, product_id });
    const result = await storage.createCustomRequest(images, comment, product_id);
...
app.get("/api/categoryuserdata", async (req, res) => {
  ...
  // Landing contact: accepts name, email, phone
  app.post("/api/landing/contact", async (req, res) => {
    try {
      ...
      const result = await storage.addLandingContact({ name, email, phone });
